...
- **状态：** complete
- 执行的操作：
  - 修复 1120px 以下响应式断点仍使用旧 4 行 grid 的问题。
  - 将标题输入改为紧凑横向行，避免把正文输入区挤出大空白。
  - 给 `styles.css` 和 `app.js` 增加版本号，避免浏览器继续使用旧缓存。
  - 调整目录选择能力判断，不再把 IndexedDB 失败等同于无法选择目录。
  - 增加目录选择失败的明确提示，包括安全策略阻止和浏览器不支持。
- 创建/修改的文件：
  - `index.html`
  - `styles.css`
  - `app.js`
  ...
  - `findings.md`
  - `progress.md`
  - `verification/layout-title-fixed-768.png`
